...
// Fork: our free-tier fingerprint quartet (bash/glob/grep/read) is appended
// to every free-tier body, so tools carry the caller tool + the quartet.
const FORK_TOOL_NAMES = ["get_weather", "bash", "glob", "grep", "read"];
...
describe("opencode Free 1.3 tool_choice auto-only", () => {
  ...
  it.each([
    ...
    for (const model of [FREE_13, `${FREE_13}(max)`]) {
      ...
      expect(out.tools.map((t) => t.name)).toEqual(FORK_TOOL_NAMES);
  ...
  it("giữ auto và absent; tools/input nguyên vẹn", () => {
    ...
    expect(autoOut.tools.map((t) => t.name)).toEqual(FORK_TOOL_NAMES);
    ...
    expect(absentOut.tools.map((t) => t.name)).toEqual(FORK_TOOL_NAMES);
  ...
  it("wire: execute gửi choice auto tới /zen/v1/responses", async () => {
    ...
    expect(sent.tools.map((t) => t.name)).toEqual(FORK_TOOL_NAMES);
